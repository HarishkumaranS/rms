...
function pending_order()
{
  if (isset($_SESSION['admin']) || isset($_SESSION['biller'])) {
    ...
    $s_no=0;
    if ($num > 0) {
      echo "<form name='myform' method='post' action='' onsubmit='return validateCheckboxes()'>";
      ...
      while ($row = mysqli_fetch_array($result_select)) {
        ...
        // <td>$payment_type</td>
        echo "<tr><th scope='row'>$s_no</th><td>$user_name</td><td>$product_name</td><td>$qty</td><td>$price</td><td>$o_date</td><td>$status</td><td><div class='toggler'>
    <input id='toggler-$s_no' class='checkbox' name='status$order_id' type='checkbox' value='$order_id'>
    <label for='toggler-$s_no'>
