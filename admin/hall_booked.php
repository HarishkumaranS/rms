...
function hall_booked()
{
    if (isset($_SESSION['admin'])) {
                    ...
                    <tr>
                        <td>
                            <div class="form-outline w-10 m-1">
                                <!-- <label for="from date" class="form-label">Booking Date</label> -->
        ...
        <td>
            <form method="post" action="hall_pdf.php">
        ...
        if (isset($_POST['submit'])) {
            if (isset($_POST['f_date']) && isset($_POST['t_date']) && !empty($_POST['t_date']) && !empty($_POST['f_date'])) {
                $f_date = $_POST['f_date'];
                echo "<input type='hidden' name='f_date' value='$f_date'>";
                $t_date = $_POST['t_date'];
                echo "<input type='hidden' name='t_date' value='$t_date'>";
                $add_qry = "where b_date between '$f_date' and '$t_date' ";
            } elseif (isset($_POST['f_date']) && !empty($_POST['f_date'])) {
                $f_date = $_POST['f_date'];
                echo "<input type='hidden' name='f_date' value='$f_date'>";
                $add_qry = "where b_date between '$f_date' and (select MAX(b_date) From event_booking) ";
            } elseif (isset($_POST['t_date']) && !empty($_POST['t_date'])) {
                $t_date = $_POST['t_date'];
                echo "<input type='hidden' name='t_date' value='$t_date'>";
                $add_qry = "where b_date<='$t_date' ";
            }
        }
        $select_qry = "SELECT *,CASE time WHEN  'morning' THEN 'Morning to Afternoon' WHEN 'evening' THEN 'Evening to Night' END AS time_of_day FROM event_booking " . $add_qry ." order by b_id DESC";
        echo "<input type='hidden' name='query' value='$select_qry'>";
        echo "</form>";
        ...
        if ($num > 0) {
            while ($row = mysqli_fetch_array($result_select)) {
                ...
                $select_qry_join = "SELECT c.cust_name,e.e_name from event e,customer c where c.cust_id=$cust_id and e_id=$even_id";
                $result_select_join = mysqli_query($con, $select_qry_join);
                $row_join = mysqli_fetch_array($result_select_join);
                $e_name = $row_join['e_name'];
                $cust_name = $row_join['cust_name'];
                $data[]=[$date,$b_date,$cust_name,$e_name,$time];
            }
            ?>
