    ...
    if (isset($_SESSION['admin']) || isset($_SESSION['biller'])) {
        ...
        <div class="navbar_heading">
            ...
            <div style="position: absolute; top: 50%; right: 0; transform: translateY(-50%);">
                <a href="index.php?add_event" class="add_btn text-decoration-none">+ ADD</a>
                <a href="index.php?view_del_event" class="delete_btn text-decoration-none">Del Product</a>
            </div>
            <?php } ?>
        </div>
        <?php
        ...
        // no categeory condition (for svg)
        if ($num > 0) {
            $data = [];
            while ($row = mysqli_fetch_array($result)) {
                ...
                $e_cost = $row['e_cost'];
                $data[] = [
                    htmlspecialchars($e_name),
                    htmlspecialchars($e_cost),
                    isset($_SESSION['admin']) ? "<a class='nav-link' href='index.php?EEid=$e_id'><i class='fa-solid fa-pen-to-square edit_color'></i></a>" : null,
                    isset($_SESSION['admin']) ? "<a onclick='del_event($e_id)'><i class='fa-solid fa-trash del_color'></i></a>" : null
                ];
            }
            ?>
