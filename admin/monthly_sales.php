...
function monthly_sales()
{
    ...
    if (isset($_SESSION['admin'])) {
        ...
        <div class="navbar_heading">
            <h4 style="margin: 0; text-align: center;"><b>Monthly Sales</b></h4>
            <div style="position: absolute; top: 50%; left: 0; transform: translateY(-50%);">
            <form action="" method="post" id="orderForm">
    <div class="checkbox-wrapper-10">
        <input type="checkbox" id="cb5" class="tgl tgl-flip" value="online" name="type"
            <?php if(isset($_POST['type']) && $_POST['type'] == "online"){echo "checked";} ?>
            onchange="delayedSubmit();">
        <label for="cb5" data-tg-off="Online" data-tg-on="Direct" class="tgl-btn"></label>
    </div> 
</form>
        ...
        if(isset($_POST['type']))
        {
        $select_qry = "SELECT * FROM user_order 
                WHERE MONTH(d_date) = MONTH(CURDATE()) 
                AND YEAR(d_date) = YEAR(CURDATE())";
        }
        else
        {
            $select_qry="SELECT * FROM order_off  WHERE MONTH(order_date) = MONTH(CURDATE()) 
